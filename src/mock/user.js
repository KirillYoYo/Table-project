	...
	'data|40-50': [
		{
			...
			name: '@name',
			lastName: '@last',
			group: Mock.Random.pick(groups).name,
			// phone: /^1[34578]\d{9}$/,
			//'age|11-99': 1,
			//address: '@county(true)',
			// address: null,
			...
			createTime: '@datetime',
			avatar () {
				return Mock.Random.image('100x100', Mock.Random.color(), '#757575', 'png', this.lastName.substr(0, 1))
			},
		},
	],
})
...
const reformatData = (function () {for (let key in usersListData.data) {
	if (usersListData.data[key]) {
		usersListData.data[key].group =  Mock.Random.pick(groups).name
	}
}
}());
